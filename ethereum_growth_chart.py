...
def get_supply_data(start_date):
    """CoinMetrics에서 이더리움 총공급량 데이터를 가져옵니다."""
    print(f"▶ 1단계: CoinMetrics에서 {start_date}부터 총공급량 데이터를 가져옵니다...")
    try:
        url = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics"
        params = {"assets": "eth", "metrics": "SplyCur", "start_time": f"{start_date}T00:00:00Z", "frequency": "1d", "page_size": 10000}
...
if __name__ == "__main__":
    ...
    price_df = get_price_data('ETH-USD', START_DATE)

    if supply_df is not None:
        supply_df.to_csv("supply_data.csv", index=False)
        print("✅ 공급량 데이터가 'supply_data.csv' 파일로 저장되었습니다.")
    if price_df is not None:
        price_df.to_csv("price_data.csv", index=False)
        print("✅ 가격 데이터가 'price_data.csv' 파일로 저장되었습니다.")
    ...
    # 그래프 전체 레이아웃 설정
    fig.update_layout(
    title_text=f'<b>Ethereum: Price vs. Total Supply (Since {START_DATE})</b>',
...
,
    ...
    # 5. HTML 파일로 저장
    file_name = f"graph_eth_supply.html"
